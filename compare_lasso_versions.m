...
for i = 1:nReps
    ...
    trash = fitlm(FM_LS_L_v9(:,i),yFM); RMSE{3,1}(i) = trash.RMSE; R2{3,1}(i) = trash.Rsquared.Ordinary; AR2{3,1}(i) = trash.Rsquared.Adjusted; SLP{3,1}(i) = trash.Coefficients.Estimate(2);
    trash = fitlm(FM_LS_Q_v9(:,i),yFM); RMSE{4,1}(i) = trash.RMSE; R2{4,1}(i) = trash.Rsquared.Ordinary; AR2{4,1}(i) = trash.Rsquared.Adjusted; SLP{4,1}(i) = trash.Coefficients.Estimate(2);
    ...
    trash = fitlm(FM_LS_L_v12(:,i),yFM); RMSE{9,1}(i) = trash.RMSE; R2{9,1}(i) = trash.Rsquared.Ordinary; AR2{9,1}(i) = trash.Rsquared.Adjusted; SLP{9,1}(i) = trash.Coefficients.Estimate(2);
    trash = fitlm(FM_LS_Q_v12(:,i),yFM); RMSE{10,1}(i) = trash.RMSE; R2{10,1}(i) = trash.Rsquared.Ordinary; AR2{10,1}(i) = trash.Rsquared.Adjusted; SLP{10,1}(i) = trash.Coefficients.Estimate(2);
    ...
    trash = fitlm(FM_RF_L_v9(:,i),yFM); RMSE{15,1}(i) = trash.RMSE; R2{15,1}(i) = trash.Rsquared.Ordinary; AR2{15,1}(i) = trash.Rsquared.Adjusted; SLP{15,1}(i) = trash.Coefficients.Estimate(2);
    trash = fitlm(FM_RF_Q_v9(:,i),yFM); RMSE{16,1}(i) = trash.RMSE; R2{16,1}(i) = trash.Rsquared.Ordinary; AR2{16,1}(i) = trash.Rsquared.Adjusted; SLP{16,1}(i) = trash.Coefficients.Estimate(2);
    ...
    trash = fitlm(WO_LS_L_v9(:,i),yWO); RMSE{3,2}(i) = trash.RMSE; R2{3,2}(i) = trash.Rsquared.Ordinary; AR2{3,2}(i) = trash.Rsquared.Adjusted; SLP{3,2}(i) = trash.Coefficients.Estimate(2);
    trash = fitlm(WO_LS_Q_v9(:,i),yWO); RMSE{4,2}(i) = trash.RMSE; R2{4,2}(i) = trash.Rsquared.Ordinary; AR2{4,2}(i) = trash.Rsquared.Adjusted; SLP{4,2}(i) = trash.Coefficients.Estimate(2);
    ...
    trash = fitlm(WO_LS_L_v12(:,i),yWO); RMSE{9,2}(i) = trash.RMSE; R2{9,2}(i) = trash.Rsquared.Ordinary; AR2{9,2}(i) = trash.Rsquared.Adjusted; SLP{9,2}(i) = trash.Coefficients.Estimate(2);
    trash = fitlm(WO_LS_Q_v12(:,i),yWO); RMSE{10,2}(i) = trash.RMSE; R2{10,2}(i) = trash.Rsquared.Ordinary; AR2{10,2}(i) = trash.Rsquared.Adjusted; SLP{10,2}(i) = trash.Coefficients.Estimate(2);
    ...
    trash = fitlm(WO_RF_L_v9(:,i),yWO); RMSE{15,2}(i) = trash.RMSE; R2{15,2}(i) = trash.Rsquared.Ordinary; AR2{15,2}(i) = trash.Rsquared.Adjusted; SLP{15,2}(i) = trash.Coefficients.Estimate(2);
    trash = fitlm(WO_RF_Q_v9(:,i),yWO); RMSE{16,2}(i) = trash.RMSE; R2{16,2}(i) = trash.Rsquared.Ordinary; AR2{16,2}(i) = trash.Rsquared.Adjusted; SLP{16,2}(i) = trash.Coefficients.Estimate(2);
